const tagList =  [
    ...
    },
    {
      ...
      AlternativeNames: ['jee']
    },
    {
      Id: '5',
      Title: 'j',
      Description: 'J is a high-level, general-purpose, high-performance array programming language.',
      AlternativeNames: []
    },
    {
      Id: '6',
      ...
      AlternativeNames: ['jquery-core', 'jquery-callback', 'jquery-after']
    }
  ];


  function createCard(tagItems) {
    return 
        <div>
          <h3>${tagItems.Id}</h3>
          <p>${tagItems.Title}</p>
          <p>${tagItems.Description}</p>
        </div>
        }

const templates = tagList.map(tag => createCard(tag));
